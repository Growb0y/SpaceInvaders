...
class KotlinInvadersView(context: Context,
    ...
    private var coin: Coin = Coin(context, size.x, size.y)
    ...
    private val maxInvaderBullets = 100
    ...
    private var coinAppearanceInterval: Long = 10000

    // Which menace sound should play next
    private var uhOrOh: Boolean = false
    // When did we last play a menacing sound
    private var lastMenaceTime = System.currentTimeMillis()
    private var lastCoinAppearance = System.currentTimeMillis()
    ...
    private fun showResultsActivity() {
        ...
        intent.putExtra("wavesPassed", waves-1)
    ...
    private fun addScore(pointsScored: Int) {
        score += pointsScored
        if (score > highScore) {
            highScore = score
        }
    ...
    private fun update(fps: Long) {
        ...
        {
            if (coin.position.top < size.y)
            {
                coin.position.top = size.y.toFloat()
            }
            if (System.currentTimeMillis() - lastCoinAppearance >= coinAppearanceInterval) {
                coin.appear()
                lastCoinAppearance = System.currentTimeMillis()
            }
        }
        ...
        if (coin.isActive) {
            if (RectF.intersects(playerShip.position, coin.position)) {
                coin.isActive = false
                addScore(100)
                soundPlayer.playSound(SoundPlayer.coinCollectionID)
            }
        }
    ...
    private fun draw() {
        ...
        if (holder.surface.isValid) {
            ...
            canvas.drawBitmap(coin.currentFrame, coin.position.left,
                coin.position.top, paint)
